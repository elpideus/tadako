...
export default class Tadako {
  ...
  ): Promise<{ page: number; maxPages: number; results: Anime[] }> {
    const $ = await Tadako.parse(`https://${this.domain}/${this.routes.list}`);
    const maxPages =
      parseInt(
        $(
          "#main .content .widget.az-list .widget-body div .paging-wrapper #paging-form .total",
        ).text(),
      ) || 1;

    const results: Anime[] = [];
    $("#main .content .widget.az-list .widget-body .items .item").each(
      (_, item) => {
        const $item = $(item);
        results.push(
          new Anime(
            `https://${Tadako.domain}${$item.find(".info .name").attr("href")}`,
            {
              ...
              releaseDate:
                DateParser.parseItalianDate(
                  $item.find(".info .name").attr("data-tippy-content") || "",
                ) || new Date(),
            },
          ),
        );
  ...
  static async search(
    ...
    const $ = await Tadako.parse(
      `https://${this.domain}/${this.routes.search}?keyword=${query}&${filterString}`,
    );

    const results: Anime[] = [];
    $("#main .content .widget .widget-body .film-list .item").each(
      (_, item) => {
        const $item = $(item);
        ...
        if ($item.find("a.poster .status .special"))
          mediaType = MediaType.SPECIAL;
    ...
    const pagePlaceholder =
      $(
        "#main .content .widget .widget-body .paging-wrapper #paging-form #page-input",
      ).attr("placeholder") || "1";
    const maxPagesText =
      $(
        "#main .content .widget .widget-body .paging-wrapper #paging-form .total",
      ).text() || "1";
